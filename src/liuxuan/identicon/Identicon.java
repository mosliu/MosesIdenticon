...
import static liuxuan.identicon.DrawUtils.gotInt;
...
public class Identicon {
    ...
    public BufferedImage create(String centerChar) {
        int cornerShapeNo = gotInt(hash, 0, 1) & 0x0f;       // corner sprite shape
        int sideShapeNo = (gotInt(hash, 0, 1) & 0xf0) >> 4;       // side sprite shape
        int centerShapeNo = gotInt(hash, 1, 1) & 0x07;   // center sprite shape


        int cornerShapeStyle = gotInt(hash, 2, 1) & 0x07; // corner sprite rotation
        int sideShapeStyle = (gotInt(hash, 2, 1) & 0x70) >> 4; // side sprite rotation
